...
def test_pii_scanner_redacts_aadhaar_pan_and_abha_id():
    ...
    processed, detections, is_skipped = CodeSentinelScanner.scan_chunk_for_pii_and_secrets(
        sample_code, "services/vault.py", "studentkare_core"
    )
...
def test_t1_weighted_portfolio_health_score_calculation():
    scorecards = [
        PortfolioScorecard(
            product_id="studentkare_core",
            product_name="StudentKare Core",
        ...
        PortfolioScorecard(
            product_id="studentkare_rppg_vitals",
            product_name="rPPG Vitals",
            tier=DataGovernanceTier.T1,
            health_score=100.0,  # T1 (1.5x weight)
            trend="STABLE",
            open_p0_count=0,
            open_p1_count=0,
    ...
    # Weighted calculation: (80 * 1.5 + 100 * 1.5) / (1.5 + 1.5) = 270 / 3.0 = 90.0
    weighted_score = CodeSentinelScanner.calculate_portfolio_health_score(scorecards)
    assert weighted_score == 90.0
...
def test_cross_product_pattern_grouping():
    findings = [
        PortfolioFinding(
            ...
            affected_repo="kktejas07/Studentkare/backend/core",
        ...
        PortfolioFinding(
            id="F2",
            product_id="studentkare_clinical_ai",
            tier=DataGovernanceTier.T1,
            severity="P1",
            rule_fingerprint="SHARED-AUTH-MISSING-CSRF",
            title="Missing CSRF validation",
            description="CSRF middleware missing",
            affected_repo="kktejas07/Studentkare/backend/services/ai_triage",
    ...
    assert "studentkare_core" in patterns[0]["affected_products"]
    assert "studentkare_clinical_ai" in patterns[0]["affected_products"]
...
def test_weekly_portfolio_digest_generation():
    ...
    digest = CodeSentinelScanner.generate_weekly_portfolio_digest(findings, {"studentkare_core": 3})
...
def test_sentinel_super_admin_endpoints(harness):
    ...
    assert len(res_port.json()['products']) >= 6
