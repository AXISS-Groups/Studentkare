...
const categoryColor = (a: string) => (c: string) => ({ skin: '#cba18f', devices: '#65a6aa', vitamins: '#9c855c', nutrition: '#a3906f', first_aid: '#c88781', ayurveda: '#8fa477', medicines: '#7f8ba0', labs: '#a38bbb' }[c] || a);
...
<OffersBanner onShop={() => { setCategory('all'); setQuery(''); setPage(0); }} /></>}
      <section className="shop-section shop-container">{content.data?.features?.length ? <div className="shop-trust-strip" aria-label="Marketplace features">{content.data.features.map(feature => <div key={feature.key}>{contentIcon(feature.icon, 23)}<span><strong>{feature.title}</strong><small>{feature.body}</small></span></div>)}</div> : null}
