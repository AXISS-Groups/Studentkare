...
export function AuthenticatedFlowScreen({ mode, next }: { mode: 'login' | 'signup'; next: RoutePath | null }) {
  ...
  const sendCode = () => mutation.run(() => apiRequest<{ targetMasked: string }>('/auth/otp/send', {
    method: 'POST', body: JSON.stringify({ identifier, channel, intent: mode === 'login' ? 'LOGIN' : 'SIGNUP' }),
  }), response => { setMasked(response.targetMasked); setResendAt(Date.now() + 45000); setCode(''); setStep(3); });
