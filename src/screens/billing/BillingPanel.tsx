...
export function BillingPanel() {
  ...
  return <div className="wf-panel">
    ...
    <DataState loading={!me && !error} error={error} retry={load}>
      {me && <div className="wf-billing-current">
        <div className="wf-card">
          <span className={`wf-status ${plus ? 'status-completed' : ''}`}>{me.status}</span>
          <h3>{me.plan.name}</h3>
          <p>{me.plan.description}</p>
          {me.organizationPlan && <p><strong>{me.organizationPlan.organization}</strong> · institutional seat access until {displayDate(me.organizationPlan.periodEnd)}</p>}
          {plus && <p><strong>Benefit allowance:</strong> {me.benefits.used} of {me.benefits.limit} used this period · ends {displayDate(me.periodEnd)}</p>}
        </div>
        <div className="wf-billing-actions">
          ...
          {free && !me.checkoutAvailable && <p className="wf-fineprint">Online payment is not connected. Contact support to activate Student Plus.</p>}
        </div>
      </div>}

      {me && me.receipts.length > 0 && <div className="wf-card">
    ...
    {me?.effectivePlanId === 'FREE' && <div className="wf-card"><h3>What Student Plus includes</h3>
      <ul className="wf-plan-benefits">{me.plan.benefits.map(benefit => <li key={benefit}><Check size={16} />{benefit}</li>)}</ul>
      <p className="wf-fineprint">Benefits are limited and partner-funded. They do not replace insurance or clinical care.</p></div>}
  </div>;
}
